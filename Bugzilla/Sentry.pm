...
use constant CONFIG => {
  ...
  logger => [
    ...
    {match => [qr/PatchReader/],                logger => 'patchreader',},
    {match => [qr/Use of uninitialized value/], logger => 'uninitialized_warning',},
  ],
};
...
sub sentry_handle_error {
  ...
  $send_to_sentry = 0 if $logger eq 'patchreader';
  ...
  eval {
    ...
    if ($user->id) {
      $user_data = {id => $user->login, name => $user->name,};
    }
  };
  ...
  my @sanitise_vars
    = qw( HTTP_COOKIE HTTP_X_BUGZILLA_PASSWORD HTTP_X_BUGZILLA_API_KEY HTTP_X_BUGZILLA_TOKEN );
