...
sub group_always_settable {
  my ($self, $group) = @_;
  my @always_fileable_groups
    = split(/\s*,\s*/, Bugzilla->params->{always_fileable_groups});
...
sub default_platform {
  ...
  if (!exists $self->{default_platform}) {
    $self->{default_platform}
      ...
      ->new($self->{default_platform_id})->name
      : undef;
  }
...
sub default_op_sys {
  ...
  if (!exists $self->{default_op_sys}) {
    $self->{default_op_sys}
      ...
      ? Bugzilla::Field::Choice->type('op_sys')->new($self->{default_op_sys_id})
      ->name
      : undef;
  }
