...
sub init_page {
  ...
  if (Bugzilla->params->{"shutdownhtml"} && !grep { $_ eq $script }
    SHUTDOWNHTML_EXEMPT)
  {
...
sub metrics {
  return $_[0]->request_cache->{metrics}
    ||= Bugzilla::Metrics::Collector->new($_[1]);
}
...
sub memcached {
  ...
  # BMO - use metrics subclass if required
  if (Bugzilla->metrics_enabled) {
    return $_[0]->request_cache->{memcached}
      ||= Bugzilla::Metrics::Memcached->_new();
  }
  else {
