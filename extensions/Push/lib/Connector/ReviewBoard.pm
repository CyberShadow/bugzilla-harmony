  ...
  return (
    {
      ...
      required => 1,
      validate => sub {
        Bugzilla::Product->new({name => $_[0]}) || die "Invalid Product ($_[0])\n";
      },
    },
    {
      name     => 'component',
    ...
    },
    {
      ...
      required => 1,
      validate => sub {
        Bugzilla::Group->new({name => $_[0]}) || die "Invalid Group ($_[0])\n";
      },
    },
    {
      name     => 'cc',
      ...
      validate => sub {
        foreach my $login (map { trim($_) } split(',', $_[0])) {
          Bugzilla::User->new({name => $login}) || die "Invalid User ($login)\n";
        }
      },
    },
  );
