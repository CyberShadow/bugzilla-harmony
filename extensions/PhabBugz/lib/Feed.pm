...
use Bugzilla::Search;
use Bugzilla::Util
  qw(diff_arrays with_writable_database with_readonly_database);
...
sub process_revision_change {
  ...
  # else bug is private.
    ...
    # groups that are mapped to bugzilla groups.
      ...
      if ($revision->view_policy =~ /^PHID-PLCY/) {
        ...
        $self->logger->debug(
          "Current policy projects: " . join(", ", @$current_projects));
      ...
      if (!$current_policy) {
        $self->logger->debug(
          "Creating new custom policy: " . join(", ", @set_projects));
  ...
  my $attachment
    = create_revision_attachment($bug, $revision->id, $revision->title,
    $timestamp);
  ...
  foreach my $attachment (@attachments) {
    ...
    $self->logger->debug(
      'Updating obsolete status on attachmment ' . $attachment->id);
  ...
  unless ($revision->status eq 'changes-planned'
    || $revision->status eq 'needs-review')
  {
    foreach my $reviewer (@{$revision->reviewers}) {
      push(@accepted_phids, $reviewer->phab_phid)
        if $reviewer->phab_review_status eq 'accepted';
      push(@denied_phids, $reviewer->phab_phid)
        if $reviewer->phab_review_status eq 'rejected';
    }
  }
  ...
  foreach my $attachment (@attachments) {
    ...
    foreach my $flag (@{$attachment->flags}) {
      ...
      if ($flag->status eq '+' && !any { $flag->setter->id == $_ }
        (@accepted_user_ids, @denied_user_ids))
      {
        push(@removed_flags, {id => $flag->id, setter => $flag->setter, status => 'X'});
      }
    }

    $flag_type ||= first { $_->name eq 'review' && $_->is_active }
    ...
    foreach my $flag_data (@denied_flags) {
      $comment
        .= $flag_data->{setter}->name . " has requested changes to the revision.\n";
    }
    foreach my $flag_data (@removed_flags) {
      $comment
        .= $flag_data->{setter}->name . " has been removed from the revision.\n";
    }
