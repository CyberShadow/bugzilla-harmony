...
sub _get_revisions {
  ...
  my $data = {queryKey => 'all', constraints => $constraints};
...
sub make_revision_public {
  ...
  return request(
    ...
    {
      transactions =>
        [{type => 'view', value => 'public'}, {type => 'edit', value => 'users'}],
...
sub set_revision_subscribers {
  ...
  my $data = {
    transactions     => [{type => 'subscribers.set', value => $subscribers}],
    objectIdentifier => $revision_phid
  };
...
sub get_project_phid {
  ...
  # Check memcache
  my $project_phid
    = $memcache->get_config({key => "phab_project_phid_" . $project});
  if (!$project_phid) {
    my $data = {queryKey => 'all', constraints => {name => $project}};
    ...
    $memcache->set_config(
      {key => "phab_project_phid_" . $project, data => $project_phid});
  }
  return $project_phid;
}
...
sub get_members_by_phid {
  ...
  my @bmo_ids;
  foreach my $user (@$result) {
    push(@bmo_ids, $user->{id}) if ($user->{phid} && $user->{phid} =~ /^PHID-USER/);
  }
...
sub get_phab_bmo_ids {
  ...
  if ($params->{ids}) {
    ...
    for (my $i = 0; $i < @bmo_ids; $i++) {
      ...
      if ($phid) {
        push(@results, {id => $bmo_ids[$i], phid => $phid});
        splice(@bmo_ids, $i, 1);
      }
    }
    $params->{ids} = \@bmo_ids;
  ...
  if ($params->{phids}) {
    ...
    for (my $i = 0; $i < @phids; $i++) {
      ...
      if ($bmo_id) {
        push(@results, {id => $bmo_id, phid => $phids[$i]});
        splice(@phids, $i, 1);
      }
    }
    $params->{phids} = \@phids;
  ...
  foreach my $user (@{$result->{result}}) {
    $memcache->set({key => "phab_user_bmo_id_" . $user->{id}, value => $user->{phid}
    });
    $memcache->set({key => "phab_user_phid_" . $user->{phid}, value => $user->{id}
    });
    push(@results, $user);
  }
...
sub add_security_sync_comments {
  ...
  my $bmo_error_message
    = (
    $num_revisions > 1 ? $num_revisions . ' revisions were' : 'One revision was')
