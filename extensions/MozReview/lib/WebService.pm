...
sub attachments {
  ...
  foreach my $attachment (@$attachments) {
    ...
    if ($attachment_id) {
      ...
      ThrowUserError(
        "mozreview_attachment_bug_mismatch",
        {bug => $bug, attachment => $attachment_obj}
      ) if $attachment_obj->bug_id != $bug->id;

      # HACK: preload same bug object.
      $attachment_obj->{bug} = $bug;

      $attachment = translate($attachment,
        Bugzilla::WebService::Bug::ATTACHMENT_MAPPED_SETTERS);

      my ($update_flags, $new_flags)
        = $flags ? extract_flags($flags, $bug, $attachment_obj) : ([], []);
      ...
      elsif (scalar @$update_flags
        && !scalar(@$new_flags)
        && !scalar keys %$attachment)
      {
      ...
      $changes
        = translate($changes, Bugzilla::WebService::Bug::ATTACHMENT_MAPPED_RETURNS);
  ...
  my %attachments_created
    = map { $_->id => $self->_attachment_to_hash($_, $params) } @created;
  my %attachments_modified
    = map { (ref $_->{id} ? $_->{id}->value : $_->{id}) => $_ } @modified;
