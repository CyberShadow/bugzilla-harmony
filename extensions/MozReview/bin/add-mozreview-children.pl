...
sub set_review_flag {
  ...
  my %params = (type_id => $flag_type->id, status => $flag_status);
...
foreach my $bug_id (@$bug_ids) {
  ...
  my $body = get($url);
  die "Error fetching review requests for bug " . $bug->id unless defined $body;
  ...
  foreach my $summary (@$summaries) {
    ...
    my $attacher
      = Bugzilla::User->new({id => $parent->{"submitter_bmo_id"}, cache => 1});
    ...
    if (defined($parent_attach)) {
      print "    Parent attachment has ID "
        . $parent_attach->id
        . ". Obsoleting it.\n";
    ...
    foreach my $child (@children) {
      ...
      foreach my $reviewer_id (@{$child->{"reviewers_bmo_ids"}}) {
        my $reviewer = Bugzilla::User->new({id => $reviewer_id, cache => 1});
      ...
      foreach my $extra_reviewer_id (keys %tmp_parent_flags) {
        my $extra_reviewer
          = Bugzilla::User->new({id => $extra_reviewer_id, cache => 1});
        my $flag = $tmp_parent_flags{$extra_reviewer_id};
        print "     Extra flag set for reviewer " . $extra_reviewer->login . "\n";
        set_review_flag($child_attach, $flag->type, $flag->status, $extra_reviewer,
          $flag->setter);
      }
